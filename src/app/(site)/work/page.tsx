...
import { getPortfolioSectionGroups, getPublishedProjects } from "@/lib/data";
import { ProjectGrid } from "@/components/site/ProjectGrid";
import { PortfolioSections } from "@/components/site/PortfolioSections";
...
export default async function WorkPage({ searchParams }: WorkPageProps) {
  ...
  const [sections, listing] = await Promise.all([
    showSections ? getPortfolioSectionGroups() : Promise.resolve([]),
    showSections
    ...
    <PageShell portfolio>
      <div className="mb-8 max-w-3xl sm:mb-12">
        ...
        <p className="mt-4 text-base leading-relaxed text-soft-black/75">
          A curated collection of branding, visual design, and creative projects
          crafted for clarity, distinction, and impact.
        </p>
      </div>
      ...
      >
        {showSections ? (
          <PortfolioSections sections={sections} />
        ) : (
          <ProjectGrid
            key={category}
            initialProjects={listing!.projects}
